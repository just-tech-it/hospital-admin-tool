    ...
    <section style={{ padding:'1.5rem' }}>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'1rem', alignItems:'center' }}>
        ...
        <div style={{ display:'flex', gap:'10px' }}>
          ...
          <select value={filterRole} onChange={e=>setFilterRole(e.target.value)} style={{ padding:'8px', borderRadius:'6px', border:'1px solid #ddd' }}>
        ...
        <tbody>
          {filteredShifts.map(shift => (
            <tr key={shift.id} style={{ borderBottom:'1px solid #f4f7f6' }}>
              <td style={{ padding:'12px', fontWeight:'500' }}>{shift.staffName}</td>
              <td><span style={{ background:'#edf2f7', padding:'4px 8px', borderRadius:'4px', fontSize:'0.85rem' }}>{shift.role}</span></td>
              <td>{shift.checkedIn ? '🟢 On-site' : '⚪ Off-site'}</td>
              <td>
                <button onClick={()=>updateShift({...shift, checkedIn:!shift.checkedIn})}
                  style={{ cursor:'pointer', padding:'6px 12px', borderRadius:'4px', border:'none', background:shift.checkedIn?'#fff5f5':'#f0fff4', color:shift.checkedIn?'#c53030':'#2f855a' }}>
